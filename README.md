...
Sematic HTML is a language that allows us to organize code via markup.

2. Name two big differences between ```display: block;``` and ```display: inline;```.

Inline keeps the items on the same line as possible. Block can have adjusted height and width.
...
Git allows us to colloborate on a single project without breaking the master copy. It also allows us to share files with each member of the team so its easier to use throughout the process.
